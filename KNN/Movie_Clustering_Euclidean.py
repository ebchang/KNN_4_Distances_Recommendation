...
for i in range(0, n_users):
    ...
    for m in range(0, 19):
        if len(tmp[m]) != 0:
            ...
            average[m] = 0
    utility_clustered_euclidean.append(average)
    
utility_clustered_euclidean = np.array(utility_clustered_euclidean)

#Find the average rating for each user and stores it in the user's object

for i in range(0, n_users):
    x = utility_clustered_euclidean[i]
    user_object_list[i].avg_r = sum(a for a in x if a > 0) / sum(a > 0 for a in x)

#Find the Euclidean distance between two users
euclidean_matrix = np.zeros((n_users, n_users))

def euclidean(x,y):
    dist1 =0
    dist2 = 0
    A = utility_clustered_euclidean[x-1]
    B = utility_clustered_euclidean[y-1]
    dist1 = sum((a)**2 for a in A if a>0)
    dist2 = sum((b) **2 for b in B if b>0)
    dist_total = math.sqrt(dist1 + dist2)
    if dist_total ==0:
        return 0
    else:
        return dist_total
...
for i in range(0, n_users):
    for j in range(0, n_users):
        if i !=j:
            euclidean_matrix[i][j] = euclidean(i+1, j+1)
            sys.stdout.write("\rGenerating Similarity Matrix [%d:%d] = %f" % (i+1, j+1, euclidean_matrix[i][j]))
            sys.stdout.flush()
            time.sleep(0.0005)
            
print(sys.stdout.write("\rGenerating Similarity Matrix [%d:%d] = %f" % (i+1, j+1, euclidean_matrix[i][j])))

print(euclidean_matrix)
def similarity_score(user1, user2):
    both_viewed = []
    
    for item in euclidean_matrix[user1]:
        if item in euclidean_matrix[user2]:
            both_viewed[item] = 1 
    ...
    for item in euclidean_matrix[user1]:
        if item in euclidean_matrix[user2]:
            sum_of_euclidean_distance.append(pow(euclidean_matrix[user1][item]))
            sum_of_euclidean_distance = sum(sum_of_euclidean_distance)
        return 1/(1+math.sqrt(sum_of_euclidean_distance))
...
for i in range(0, n_users):
    for j in range(0, n_users):
        if i !=j:
            ...
            sys.stdout.flush()
            time.sleep(0.00005)
print("\rGenerating Similarity Matrix [%d:%d] = %f" % (i+1, j+1,euclidean_matrix[i][j]))
...
def norm():
    ...
    for i in range(0, n_users):
        for j in range(0, 19):
            if utility_clustered_euclidean[i][j] != 0:
                normalize[i][j] = utility_clustered_euclidean[i][j] - user_object_list[i].avg_r
            else:
...
def guess(user_id, i_id, top_n):
    ...
    for i in range(0, n_users):
        if i+1 != user_id:
            similarity.append(euclidean_matrix[user_id-1][i])
    temp = norm()
...
utility_copy = np.copy(utility_clustered_euclidean)
for i in range(0, n_users):
    for j in range(0, 19):
...
true = []
pred = []
f = open('test_euclidean_15K.txt', 'w')
for i in range(0, n_users):
    for j in range(0, n_items):
