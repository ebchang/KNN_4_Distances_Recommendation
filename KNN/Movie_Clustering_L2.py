# -*- coding: utf-8 -*-
"""
Created on Wed Feb  3 13:58:32 2016
...
for i in range(0, n_users):
    ...
    for m in range(0, 19):
        if len(tmp[m]) != 0:
            ...
            average[m] = 0
    utility_clustered_L2.append(average)

utility_clustered_L2 = np.array(utility_clustered_L2)



#Find the L2MAX distance
L2_matrix = np.zeros((n_users, n_users))
#Y = pdist(X, 'cityblock')
def L2(x,y):
    distance1 =0
    distance2 = 0
    A = utility_clustered_L2[x-1]
    B = utility_clustered_L2[y-1]
    distance1 = sum((a) for a in A if a>0)
    distance2 = sum((b)  for b in B if b>0)
    total_dist = abs(distance1 - distance2)
    if total_dist ==0:
        return 0
    else:
        return np.amax(total_dist)

for i in range(0, n_users):
    for j in range(0, n_users):
        if i !=j:
            L2_matrix[i][j] = L2(i+1, j+1)
            sys.stdout.write("\rGenerating Similarity Matrix [%d:%d] = %f" % (i+1, j+1, L2_matrix[i][j]))
            sys.stdout.flush()
            time.sleep(0.0005)
            
print(sys.stdout.write("\rGenerating Similarity Matrix [%d:%d] = %f" % (i+1, j+1, L2_matrix[i][j])))

print(L2_matrix)
def similarity_score_L2(user1, user2):
    both_viewed = []
    
    for item in L2_matrix[user1]:
        if item in L2_matrix[user2]:
            both_viewed[item] = 1 
    ...
    for item in L2_matrix[user1]:
        if item in L2_matrix[user2]:
            sum_of_L2_distance.append(pow(L2_matrix[user1][item]))
            sum_of_L2_distance = sum(sum_of_L2_distance)
        return 1/(1+math.sqrt(sum_of_L2_distance))
...
for i in range(0, n_users):
    for j in range(0, n_users):
        if i !=j:
            ...
            sys.stdout.flush()
            time.sleep(0.00005)
print("\rGenerating Similarity Matrix [%d:%d] = %f" % (i+1, j+1,L2_matrix[i][j]))
...
def norm_L2():
    ...
    for i in range(0, n_users):
        for j in range(0, 19):
            if utility_clustered_L2[i][j] != 0:
                normalize[i][j] = utility_clustered_L2[i][j] - user_object_list[i].avg_r
            else:
                normalize[i][j] = float('Inf')
    return normalize

def guess_L2(user_id, i_id, top_n):
    similarity_L2 = []
    for i in range(0, n_users):
        if i+1 != user_id:
            similarity_L2.append(L2_matrix[user_id-1][i])
    temp = norm_L2()
    temp = np.delete(temp, user_id-1, 0)
    top = [x for (y,x) in sorted(zip(similarity_L2,temp), key=lambda pair: pair[0], reverse=True)]
    s = 0
    c = 0
...
utility_copy_L2 = np.copy(utility_clustered_L2)
for i in range(0, n_users):
    for j in range(0, 19):
        if utility_copy_L2[i][j] == 0:
            sys.stdout.write("\rGuessing [User:Rating] = [%d:%d]" % (i, j))
            sys.stdout.flush()
            time.sleep(0.00005)
            utility_copy_L2[i][j] = guess_L2(i+1, j+1, 150)
print ("\rGuessing [User:Rating] = [%d:%d]" % (i, j))

print (utility_copy_L2)

pickle.dump( utility_copy_L2, open("utility_matrix_L2.pkl", "wb"))

# Predict ratings for u.test and find the mean absolute error
true = []
pred = []
f = open('test_L2_15K.txt', 'w')
for i in range(0, n_users):
    for j in range(0, n_items):
        if test_matrix[i][j] > 0:
            f.write("%d, %d, %.4f\n" % (i+1, j+1, utility_copy_L2[i][cluster.labels_[j]-1]))
            true.append(test_matrix[i][j])
            pred.append(utility_copy_L2[i][cluster.labels_[j]-1])
f.close()
